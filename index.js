...
app.get("/", (req, res) => res.json({ status: "OK", message: "Primer Latino v6.2 – DEFINITIVA" }));
...
// META PELÍCULAS Y SERIES (igual que tenías)
...
app.get("/realdebrid=:token/stream/:type/:id.json", async (req, res) => {
  ...
  if (cache.has(hash) && Date.now() < cache.get(hash).expires) {
    console.log("CACHÉ GLOBAL – Instantáneo");
  ...
  try {
    ...
    if (!torrentInfo) {
      console.log(`Subiendo nuevo torrent ${hash}`);
...
app.listen(PORT, () => console.log(`Primer Latino v6.2 DEFINITIVA corriendo en puerto ${PORT}`));